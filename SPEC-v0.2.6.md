# CLI 自动升级 —— 项目规格 SPEC v0.2.6

> **版本**：SPEC-v0.2.6（2026-09-16）。
> **变更摘要（v0.2.5 → v0.2.6）**：架构不动。据 GPT 对 v0.2.4 的二次评审复核后修订——采纳时间字段例外说明、LICENSE 事实矛盾修正（仓库实有 AGPL-3.0）、威胁模型声明、checksum 解析 MUST 规则、state 文件 runId 关联、斜杠约定澄清；驳回"路径插空"（GPT 渲染误读，spec 实际无空格）与"交付物三层矛盾"定性（系评审阶段状态）。
...
**关于 LICENSE（修正事实矛盾）**：仓库根含 `LICENSE`（AGPL-3.0），系 initial commit 随归档 legacy 脚本（`.old/`）带入，**适用于 `.old/` 下历史代码**。本项目主体（SPEC + SOP + `*-v0.2.6.ps1` 新脚本）不面向外部发行、无贡献指南、不发布包；AGPL-3.0 不改变本流程的"个人维护"性质。
...
3. **脚本**（`*-v0.2.6.ps1`）——模块化实现。**当前状态：待产出**（评审通过后产出，穿插 re-probe）。
...
├── SPEC-v0.2.6.md                    # 本文件（契约）
├── cli-autoupdate-sop.md             # 执行 SOP（给定时 agent，待产出）
├── cli-common-v0.2.6.ps1             # 共享库
...
├── sync-v0.2.6.ps1                   # 共享单脚本；每 CLI copy 独立 try/catch
...
所有 JSON 顶层带 `specVersion`（`"0.2.6"`）、`name`。时间内部 UTC，展示 UTC+08:00。**完整字段级 schema 见 §5.9（与示例一一对应）**。
...
```json
{ "specVersion":"0.2.6", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.6", "name":"opencode", "channel":"github-binary", "runAt":"...", "runId":"<SHOULD>",
...
- **github-binary 通道**：`releases/latest` → tag_name（去 `v`）+ assets 筛 `windows-x64.zip`；`sourceUrl`=加速地址、`directUrl`=直连地址、`checksumAssetUrl`=`*.sha256`/`checksums.txt`（存在则记录，null 则无）。
  ...
  - **下载完整性策略**：
    ...
    3. checksum asset 不存在 → SHOULD 双 hash 比对（反代+直连两份独立下载，SHA256 一致才接受）；不一致 → `DOWNLOAD_FAIL|<cli> dual-hash-mismatch`。MAY 偏离（直连不通时仅用反代 + `integrityNote:single-source`），须在 sync.json.entries 记录。
...
```json
{ "specVersion":"0.2.6", "name":"codex", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.6", "name":"codex", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.6", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.6", "name":"<cli-autoupdate>", "runId":"<uuid>", "startAt":"<UTCISO>", "archivedTo":"archive\\<ts>" }
```
注：`name` 此处为任务名（非 CLI 名），`startAt` 替代 `runAt`（表本轮起点）。

### 5.7 `<cli>.pin`（可选，手动锁定目标版本）
```json
{ "specVersion":"0.2.6", "name":"codex", "pinVersion":"0.152.0", "reason":"user manual rollback", "setAt":"..." }
```
...
**不防什么（明确边界）**：
- **上游发布账号/Release 被攻破**：攻击者同时替换 zip 与 checksum asset，本方案无法检出——需发布者 RSA/Ed25519 签名，本项目明确不做。
...
| `upgrade-<cli>.ps1` | 读 local+remote → 判 behind（§7.3 if-elif）；ACTIONABLE=`mise upgrade <tool>@latest`，REPAIR=`mise install <tool>@<ver> --force` | 读 remote → 下载（§5.2 完整性策略 + checksum 解析）→ **ZipSlip 安全解压** → staging exe 计算 sha256 |
| `verify-<cli>.ps1` | 重扫 mise installs → 全字段 + runId(MUST) + sha256 SHOULD | 跑 staging exe `--version` → 全字段 + runId(MUST) + sha256 |
| `sync-v0.2.6.ps1`（共享） | 遍历三 CLI verified.json → 三重约束判定 → copy + 字节/sha256 复核 + `.previous` 备份 + 证据链 + summary 汇总 | 同左 |
...
### 7.4 版本号
共享库与 sync 文件名含 `-v0.2.6`；probe/upgrade/verify 每 CLI 一份，首行 `$ScriptVersion='0.2.6'` + `# SPEC: v0.2.6`。
...
| 维度 | Software_Update_Monitor v0.2.2 | Release-Monitor | 本项目 v0.2.6 |
|---|---|---|---|
...
7. 现有散脚本（`.old/`）→ 按 §7 模块清单拆分重命名（-v0.2.6 / $ScriptVersion）。`.old/` legacy 脚本受仓库 AGPL-3.0 LICENSE 约束，新脚本沿用同 LICENSE（仓库既有）。
...
8. **时钟回拨**：凭证新鲜度已加 runId 主判据兜底；runId 缺失时 runAt 兜底，极端时钟异常仍可能误判（已知边界）。
9. **上游真实性不在保护范围**：威胁模型已声明（§5.10），上游账号被攻破时 zip+checksum 同被替换无法检出，属信任前提。
...
- **验收 checklist（下一步，SOP/测试阶段产出）**：每条 MUST/SHOULD 拆成"条款 → 可观测证据（哪个 json 字段/stdout 标记/文件副作用）→ 测试用例"，与 §15 对齐。
...
| v0.2.6 | 2026-09-16 | GPT 二次评审复核（对 v0.2.4）：时间字段例外说明、LICENSE 事实矛盾修正（仓库实有 AGPL-3.0）、威胁模型声明、checksum 解析 MUST 规则、state 文件 runId 关联（SHOULD）、斜杠约定澄清；驳回"路径插空"（渲染误读）与"交付物三层矛盾"（阶段状态） | `SPEC-v0.2.6.md`（本文件） |
