# CLI 自动升级 —— 项目规格 SPEC v0.2.16

> **版本**：SPEC-v0.2.16（2026-09-17）。
> **变更摘要（v0.2.15 → v0.2.16）**：架构不动。据 anthropic 独立审计（对 v0.2.15，`.supervisor/spec-v0.2.15-anthropic-review.md`）独立核实，16 项意见：**采纳 15、驳回 1**（D1——审计对 §14 编号分布的事实声明错误）。全为契约层补全/消歧/防御性补全，不改判定语义主轴（仅 B1 收紧 TARGET_PRERELEASE 判据，方向更安全）：
> - P1 **A1**（采纳）：§6 陈锁接管伪代码 `$proc.StartTime` 读取未在 try/catch 内——`Get-Process` 成功但跨用户/受保护进程取 `StartTime` 可抛 `Win32Exception: Access is denied`（Windows 对仅 `PROCESS_QUERY_LIMITED_INFORMATION` 的进程取 `StartTime` 需更高权限），未捕获会绕过整套 LOCKED/TAKEOVER 体系致脚本异常终止无标记输出（比"不确定→LOCKED"更差——连诊断标记都不输出）。修复：把 `.StartTime` 读取与 Ticks 比较纳入 try/catch，异常一律归入 `$lookupUnknown = $true`，与 Get-Process 调用异常同等对待。
> - P2 **A2**（采纳）：§5.4 三重约束②原文专属 opencode，mise 通道语义空缺——mise 通道 sha256 全程 SHOULD，若某轮 sha256=null，②无从校验、隐性跳过。修复：§5.4 显式补一句"mise 通道：双侧 sha256 均存在则同样校验一致性；字段缺失（SHOULD 未算）则 ② 视为跳过（依赖 mise 自身完整性保证，§1 原则12）"。
> - P1 **B1**（采纳）：§7.3 条件3 TARGET_PRERELEASE 完全基于 Compare-SemVer 字符串启发式（含 `-` 即 prerelease），与 GitHub API `prerelease` 布尔字段脱节——上游给稳定版打含 `-` 但语义非预发布的 tag（如 `v1.2.3-lts`/`v1.2.3-hotfix`），即便 API `prerelease=false` 本系统仍误判 TARGET_PRERELEASE 并永久拒绝升级，叠加 §14-11 致 broken 本地永久无法自愈。修复：github-binary 通道条件3 优先信任 API `prerelease` 字段，字符串启发式仅作 mise 通道（无官方 prerelease 字段可用时）的兜底；§14-11 补充此更真实的触发场景。
> - P2 **C1**（采纳）：§6"最坏 30min 停摆窗口"表述只在持锁进程最终自然退出前提下成立——进程存活但挂起（网络 IO 死等/死循环，beat 停更但进程未死）时 PID+StartTime 校验持续判"仍存活→LOCKED"，导致无限期停摆（与 §6 锁恢复手册作为硬约束章节的存在本身矛盾——若真 30min 自愈无需人工清锁手册）。修复：改为"若持锁进程确实终止，最坏 30 分钟自动恢复；若进程存活但挂起，无自动上限，需人工介入"。
> - P1 **E1**（采纳）：§10.5 卸载触发只定义了置 `uninstalled=true` 目标状态，未规定 `npm uninstall -g opencode` 命令本身失败（非零退出码/npm 缺失/权限拒绝）时处置——若失败仍置 true，因"一旦 true 不回退"（CB-P2-1 冻结规则）造成状态与实际永久不符、且再也不会重试卸载。修复：MUST 检查退出码；仅成功才置 true；失败则保持 false、记录 fetch_run.log，等待下一达标轮次重试。
> - P2 **E2**（采纳）：§10.5 `npm uninstall -g opencode` 未声明是否绕开 PATH——本项目反复强调"harness 不依赖 PATH/mise shim"（§2 末行），Cherry 沙箱下 npm 装在非默认位置（`Get-NpmPrefix()` 已在 §7.1 定义精确值），依赖环境 PATH 解析存在操作到错误 npm 实例或找不到目标包的风险。修复：显式要求使用 `Get-NpmPrefix()` 定位 npm 全局位置调用，或显式传 `--prefix`。
> - P1 **F1**（采纳）：§10.5 D5"fatal 中断→不更新计数（保持旧值）"是粗粒度规则，其自称一致的 §8.1 规则实为"sync 内部 fatal 时，若 opencode 条目已在 fatal 前处理过，仍应更新"的细粒度规则——两处文字本身没有完全对齐，容易让只读 §10.5 的实现者遗漏"fatal 前已处理则更新"分支。修复：把 §8.1 的细粒度表述完整搬到 §10.5，而非仅引用。
> - P2 **G1**（采纳）：§7.3 九分支"else 正常不可达"隐含依赖 `healthy=false ⟺ healthDetail∈{broken, probe-error}` 这一相关性，但该相关性从未被列为 MUST 约束或纳入原子写入的 schema 校验范围——若实现 bug 打破该相关性，坏状态会静默滑入版本比较分支（6-9）而不是落入 `RUNTIME_ERROR|branch-undefined`，掩盖真实故障。修复：§5.1 显式声明该不变量并纳入回读校验。
> - P2 **H1**（采纳）：`Get-MiseExePath() = C:\Users\JasonPC\.cherrystudio\bin\mise.exe` 而同组 `Get-MiseInstallsDir`/`Get-NpmPrefix`/`MISE_DATA_DIR` 均在 `E:\Users\WIN_11\...`，跨盘跨用户缺成因说明，易被后续审计者误判为文档拼接错误。修复：§7.1 补一句"mise 本体二进制装在 Cherry Studio 安装用户（JasonPC）的 `%LOCALAPPDATA%` 下；mise 数据/installs 装在 Cherry 注入环境的运行用户（WIN_11）的 `%APPDATA%` 下——Cherry Studio 安装/运行用户分离的设计"。
> - P2 **H2**（采纳）：§7.2 `mise upgrade <tool>@latest` 语法未见"已实测"标注——相较文档中其他命令/路径反复强调的实测时间戳，此处的 mise 命令语法缺少同等级别的验证痕迹，而 mise 官方 `upgrade` 子命令通常不接受 `@version` 直接指定目标版本（该语法更常见于 `install`/`use`）。修复：§7.2 标注"实现期 MUST 实测 `mise upgrade <tool>@latest` 语法接受性；若不支持则改用 `mise install <tool>@<remote.latest> --force`（与 REPAIR 同语法）"。
> - P2 **H3**（采纳）：§6"续锁（后续每段）"中的"段"粒度未精确定义——若为粗粒度（顶层 5 大阶段），3 个 CLI 在 upgrade 阶段均接近 600s 超时的最坏情形下，单阶段耗时可逼近 30min 阈值，使"stale"在非异常场景下也可能出现。修复：§6 续锁段明确"段=每个脚本调用（每个 .ps1 文件运行一次即续锁一次）"——archive-state 段、每个 probe-local/probe-remote 调用、每个 upgrade 调用、每个 verify 调用、sync 段均独立续锁一次。
> - P2 **H4**（采纳）：§5 已规定 token"MUST NOT 写入状态文件/日志/推送平台"，但未提示底层 HTTP 客户端异常信息（含请求头）可能在被 catch 记录到 fetch_run.log 时间接携带 Authorization。修复：§5 补"错误日志记录前 MUST 脱敏 Authorization 头（替换为 `Authorization: <redacted>`）"。
> - P2 **H5**（采纳）：§5.2 API digest 解析规则只规定"digest 为 null 或字段缺失→视为不可用"，未覆盖"digest 存在但前缀非 `sha256:`"（未来 GitHub 若支持其他摘要算法）的情形。修复：§5.2 补"digest 存在但前缀非 `sha256:` → 视为不可用，回退 checksum asset（策略 3）"。
> - P2 **H6**（采纳）：`sync.json.entries[].reason` 无正式枚举，示例中 `"reason":"no fresh verified-success / pinned-mismatch / already-current"` 明显是文档占位而非真实取值——全文未给出该字段的正式枚举集合。修复：§5.5/§5.9 定义 reason 受控枚举：skip 类 `{no-fresh-credential, pinned-mismatch, already-current}`；target-locked 类 reason 自由文本但必填。
> - P2 **H7**（采纳）：§13 项7"拆分重命名"措辞暗示新脚本可能复用 `.old/` 函数逻辑，触发 AGPL-3.0 传染范围考量。修复：§13 项7 补"新脚本实现期 MUST 明确：是否对 `.old/` 代码有实质复用；若有，新脚本沿用 AGPL-3.0 不变；若纯重写，仍可保留 AGPL-3.0 或评估切换"。
> - **D1（驳回）**：审计称"§14 已闭合列表用 1–6、已知风险列表用 2,3,8,10,11,12，编号 2/3 跨子节冲突（已闭合的 2=current-run/run.lock 一致性、3=npm 卸载判据；风险声明的 2=反代可靠性、3=runId 锁模型细节）"——**事实核查证伪**：实际 §14 已闭合列表编号为 `{1,4,5,6,7,9}`（不含 2、3），已知风险列表编号为 `{2,3,8,10,11,12}`，两集合不相交。审计所称"已闭合 2=current-run 一致性、3=npm 卸载判据"在 §14 文本中不存在——实际是 4=current-run 一致性、5=npm 卸载判据。CH-P2-3 已明确"历史连续编号、跨组不连续"为有意设计，编号作为历轮审计与正文交叉引用锚点保留。审计未对 §14 实际文本作逐项核实即下结论，本项不构成有效审计意见。
...
> **严重度评级**：P0=架构/安全/数据损坏级；P1=真实 bug/语义不一致/状态与实际不符；P2=文档澄清/防御性补全/待实测标注。
...
**关于 LICENSE（修正事实矛盾）**：仓库根含 `LICENSE`（AGPL-3.0），系 initial commit 随归档 legacy 脚本（`.old/`）带入，**适用于 `.old/` 下历史代码**。本项目主体（SPEC + SOP + `*-v0.2.16.ps1` 新脚本）不面向外部发行、无贡献指南、不发布包；AGPL-3.0 不改变本流程的"个人维护"性质。
...
3. **脚本**（`*-v0.2.16.ps1`）——模块化实现。**当前状态：待产出**（评审通过后产出，穿插 re-probe）。

> 工作区当前含 SPEC 系列文档、验收 checklist 候选稿（`cli-autoupdate-acceptance-checklist-candidate.md`）、LICENSE，及历轮评审/工具目录（`.supervisor/`、`.codebuddy/`、`.backup/` 等，均被 `.gitignore` 排除不跟踪）；`.old/` 归档 legacy 脚本亦在 `.gitignore` 内。SOP 与新脚本尚未产出，属评审阶段正常状态，非交付物定义矛盾。（P2 v0.2.13 CB-P3-3：修正 v0.2.12"仅含 SPEC 系列文档 + `.old/` + LICENSE"的不实声明）
...
| 11 | 凭证新鲜度（收紧判据） | 新鲜度 = `runId 与 current-run.runId 匹配`（**唯一必要条件**）；仅 runId **字段缺失**（非不匹配）时回退 `runAt ≥ startAt`（兼容无 runId 旧凭证）。runId 存在但不匹配 → 恒不新鲜（runAt 不放行，防时钟回拨放过跨轮旧凭证）。P1 v0.2.13 CB-P2-4 收紧 |
...
| 层 | 位置 | 职责 | 谁能写 |
|---|---|---|---|
| **Staging** | claude/codex：`E:\Users\WIN_11\AppData\Roaming\CherryStudio\Toolchain\mise\installs\<tool>\<ver>\`（P2 v0.2.15 GPT-P0-1：省略号消歧义，精确值/来源/失败语义见 §7.1 Cherry mise 环境契约）；opencode：`<workspace>\staging\opencode\<ver>\` | 下载/升级/验证落点 | upgrade 脚本（运行锁内） |
...
├── SPEC-v0.2.16.md                   # 本文件（契约）
├── cli-autoupdate-sop.md             # 执行 SOP（给定时 agent，待产出）
├── cli-common-v0.2.16.ps1            # 共享库
...
├── sync-v0.2.16.ps1                  # 共享单脚本；每 CLI copy 独立 try/catch
...
│   ├── run.lock                     # 争锁文件（JSON；字段 schema §5.9，P2 v0.2.14 CH-P1-1）
│   ├── fetch_run.log
│   ├── <cli>.pin                     # 可选：锁定目标版本
...
> ```
> 上述路径均无空格。**大小写约定（P2 v0.2.13 CB-P3-4）**：实机目录为 `D:\AI\Programs\CLI\Codex\`（大写 C，2026-09-16 实测），spec 文本统一小写 `codex` 为书写约定——Windows 文件系统大小写不敏感，任意大小写等价，无功能影响。agent 调 `-File` 时改用正斜杠（如 `D:/AI/Programs/CLI/claude/claude.exe`）。
>
> **P2 v0.2.15 GPT-P0-1**：末行（mise installs 根）为本轮新增 canonical 行——该行即 claude/codex staging 根（原有末两行 workspace state/staging 为既有内容），精确值来源与失败语义见 §7.1 Cherry mise 环境契约（实测复核 2026-09-17：`installs\{claude,codex}\` 就位；claude=`installs\claude\2.1.273\claude.exe`、codex=`installs\codex\0.154.0\bin\codex.exe`）。
...
所有 state/*.json 数据文件顶层带 `specVersion`（`"0.2.16"`）、`name`（P2 v0.2.14 CH-P1-1：`run.lock` 为 §6 锁文件——存储格式亦为 JSON，但不带 specVersion/name、不参与 §11 归档，字段 schema 见 §5.9 run.lock 行）。时间内部 UTC，展示 UTC+08:00。**完整字段级 schema 见 §5.9（与示例一一对应）**。
...
- **GitHub API 认证**：probe-remote 调 GitHub API SHOULD 用认证 token 提额（未认证 60 次/小时/IP，认证 5000 次/小时）。本环境 `gh` 已认证（demonpiapia 账户）。token MAY 从 `gh auth token` 读取或环境变量注入，**MUST NOT 写入状态文件/日志/推送平台**。**v0.2.16 H4（防御性补全）**：底层 HTTP 客户端异常信息（含请求头）可能在被 catch 记录到 fetch_run.log 时间接携带 Authorization 头——错误日志记录前 MUST 脱敏 Authorization 头（替换为 `Authorization: <redacted>`）；MUST NOT 将完整 Authorization 头/查询参数中的 token 写入 fetch_run.log 或 sync.json.entries[].integrityNote 等任何持久化字段。
...
```json
{ "specVersion":"0.2.16", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<SHOULD>",
...
- **mise 通道同语义（P2 v0.2.15 GPT-P0-1）**：`installs\<tool>\` 不存在或为空 → 同上形态（`version=null`/`broken`）→ upgrade 走 REPAIR 通道；mise 环境本身不可用（`Get-MiseExePath` 缺失 / mise 子进程非零退出）→ REPAIR 失败或 remote 失败沿既有 `UPGRADE_FAIL|` / `REMOTE_FAIL|` 通道上报，**不新增标记**（§7.1 Cherry mise 环境契约）。

### 5.2 `<name>-remote.json`（probe-remote 产出）+ 下载完整性策略
```json
{ "specVersion":"0.2.16", "name":"opencode", "channel":"github-binary", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.16", "name":"codex", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.16", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<本轮uuid, MUST>",
  "version":"0.154.0", "exePath":"...", "bytes":298169136,
  "sha256":"<opencode必填，mise SHOULD>",
  "healthy":true, "matchesTarget":true }
```
- **晋升三重约束**：① 新鲜度（P1 v0.2.13 CB-P2-4 收紧：`runId == current-run.runId` 为**唯一必要条件**；仅 runId **字段缺失**时回退 `runAt ≥ current-run.startAt`——runId 存在但不匹配则恒不新鲜，runAt 不放行，防时钟回拨放过跨轮旧凭证）② 完整性（opencode：`verified.sha256` 非空，且 sync 复算 `sourceSha256 == verified.sha256`——同一 staging exe 的 hash 一致，凭证链成立；zip 下载完整性已由 upgrade 阶段 §5.2 策略闭合，sync 不再校验 zip hash）。**v0.2.16 A2（mise 通道语义补全）**：mise 通道——双侧 sha256 均存在（local.json.sha256 与 verified.json.sha256）则同样校验 `sourceSha256 == verified.sha256` 一致性；若字段缺失（mise SHOULD 未算）则 ② 视为跳过（依赖 mise 自身完整性保证，§1 原则12），完整性①③ 仍 MUST 满足。③ pin 未冲突（§5.7）。
...
- **`matchesTarget` 比较基准（P1 v0.2.8，回应 anthropic）**：`matchesTarget` 表示 verify 时实测版本是否与本轮判定目标一致，比较基准**按 upgrade 决策分支**取定，不统一用某单一来源：
...
```json
{ "specVersion":"0.2.16", "name":"sync", "runAt":"...", "runId":"<本轮uuid, MUST>",
  "runStatus":"success",
...
- `action`：`copy` / `skip` / `target-locked`（P1 v0.2.13 CB-P2-3：`SYNC_TARGET_LOCKED` 场景钉死 `action="target-locked"` + `reason` 必填，与 §8.1 对齐）。

### 5.6 `current-run.json`（archive-state 首步生成）
```json
{ "specVersion":"0.2.16", "name":"<cli-autoupdate>", "runId":"<uuid>", "startAt":"<UTCISO>", "archivedTo":"archive\\<ts>" }
```
注：`name` 此处为**任务名（非 CLI 名），固定值 `<cli-autoupdate>`**；`startAt` 替代 `runAt`（表本轮起点）。P2 v0.2.15 GPT-P0-3：本字段与其他文件的 CLI 名语义不同，**验收断言 MUST 分开编写**（schema 独立行见 §5.9）。

### 5.7 `<cli>.pin`（可选，手动锁定目标版本）
```json
{ "specVersion":"0.2.16", "name":"codex", "pinVersion":"0.152.0", "reason":"user manual rollback", "setAt":"..." }
```
...
**specVersion 不匹配处理（P1 v0.2.8，回应 anthropic——使 checklist A 节断言有契约依据）**：
- 读入 state 文件时若 `specVersion != 当前实现版本`（如读到 `"0.2.14"` 而本轮跑 `"0.2.16"`），**视为陈旧文件**：MUST NOT 作本轮决策依据消费（即本轮 probe/upgrade/verify/sync 的判定**不以旧 specVersion 文件内容为准**）。
- **行为分级**：
  - probe-local/probe-remote 类：本轮重新探测会自然覆盖旧文件（§5.1/§5.2 产出路径），不 fatal 终止——终止反破坏可用性（本任务目标是"让 CLI 可用"，旧 specVersion 不代表数据损坏，仅版本漂移）。
  - verified.json：旧 specVersion 的 verified.json **不作本轮晋升凭证**——sync 晋升三重约束①新鲜度（runId 唯一必要判据，仅字段缺失回退 runAt≥startAt）天然拦住跨轮旧凭证（旧轮 runId ≠ 本轮），specVersion 检查为第二道保险（防跨 specVersion 陈旧文件被误消费——P1 v0.2.13 CB-P2-4 收紧后 runId 不匹配即不新鲜，同 specVersion 跨轮旧凭证已被 runId 判据拦住，specVersion 检查为纵深防御）。
...
| 文件 | 字段 | 类型 | 必填 | 枚举/说明 |
|---|---|---|---|---|
| 全部 | specVersion | string | 是 | `"0.2.16"` |
...
| | expectedSha256 | string\|null | 条件 | github-binary；64hex（剥前缀）；digest 主路径或 checksum 路径（P2 v0.2.13 CB-P3-5）命中时落值，二者皆无=null |
...
| | healthy | bool | 是 | |
| sync.json | runStatus | string | 是 | success/failed（v0.2.12 P2-1 引入；P1 v0.2.13 CB-P2-2：sync 本次执行终态，"本轮完整成功"须联判 runId，见 §5.5） |
...
| | entries[].reason | string\|null | 条件 | skip/target-locked 必填（P1 v0.2.13 CB-P2-3；v0.2.16 H6：skip 类受控枚举 `{no-fresh-credential, pinned-mismatch, already-current}`，target-locked 类自由文本但必填） |
| | entries[].ok | bool\|null | 条件 | copy 必填 |
| current-run.json | name | string | 是 | P2 v0.2.15 GPT-P0-3：**任务名（非 CLI 名），固定 `<cli-autoupdate>`**——与其他文件的 CLI 名语义不同，验收断言 MUST 分开编写 |
| current-run.json | startAt | ISO | 是 | 替代 runAt（格式见 §5 时间字段序列化格式） |
| | archivedTo | string | 是 | |
| pin | pinVersion | string | 是 | |
| | reason | string | 否 | |
| | setAt | ISO | 否 | 替代 runAt |
| opencode-npm-fallback.json | consecutiveSyncSuccess | int | 是 | 0-10；连续 sync 成功轮数（§10.5）；P1 v0.2.13 CB-P2-1：`uninstalled==true` 后**完全冻结**（成功不增、失败不归零，定格 10） |
| | lastUpdatedAt | ISO | 是 | sync 末步更新时间 |
| | uninstalled | bool | 是 | true=已执行 npm uninstall（幂等防重复） |
| run.lock | runId | string(uuid) | 是 | 与 current-run.runId 同值（§6 ownership 判据）；P2 v0.2.14 CH-P1-1 |
| | start | ISO | 是 | 本轮起点（与 current-run.startAt 同语义） |
| | beat | ISO | 是 | 最近续锁心跳时间（UTC，'o' 序列化） |
| | pid | int | 是 | 争锁进程 PID（陈锁死亡判据之一，§6） |
| | processStartTimeUtc | string | 是 | UTC 'o' round-trip，恒 7 位小数 + `Z` 后缀（DateTime 序列化实测自洽，P2 v0.2.14 CH-P1-3）；陈锁接管比对值（§6） |

> **run.lock 格式声明（P2 v0.2.14 CH-P1-1）**：run.lock 为 §6 运行时锁文件，存储格式 = **JSON**（UTF-8；ConvertTo-Json 序列化 / ConvertFrom-Json 反序列化——§6 伪代码 `$lock.<field>` 访问语义的前提）。作为互斥结构**不带 specVersion/name**（不适用本表"全部"行），不参与 §11 归档。字段见上表 run.lock 行；`processStartTimeUtc` 格式契约（UTC 'o'、7 位小数）自 CB-P1-2 钉死，本表为其 schema 级落点，验收断言不再漏 run.lock。
>
> **时间字段格式总注（P2 v0.2.15 GPT-P1-1）**：本表所有 ISO 类型时间字段（`runAt`/`startAt`/`setAt`/`lastUpdatedAt`/`beat`/`mtime`）MUST 以 UTC 'o' round-trip（恒 7 位小数 + `Z`）序列化——格式契约与比较规则见 §5 时间字段序列化格式；`run.lock.processStartTimeUtc` 原有契约（CB-P1-2/CH-P1-3）不变，与本注同源。
...
elseif ($null -eq $proc) {
    ...
    # P1 v0.2.16 A1（采纳 anthropic）：Get-Process 成功不代表 .StartTime 访问成功——
...
- **不变量（v0.2.13 修订 + v0.2.16 A1）**：`TAKEOVER` 仅在两个**已证明**死亡的分型发出——"查询正常且 PID 无活跃进程"（`ProcessCommandException` 分型）或"PID 命中但 tick 不匹配（锁值已过格式校验）"。`LOCKED` 涵盖"同一实例仍存活""字段缺失/格式不合契约""查询异常未证明死亡""StartTime 访问异常未证明死亡（v0.2.16 A1：Get-Process 成功但 .StartTime 抛 Win32Exception 等，与查询异常同等对待）"——均 fail-closed 不抢。异常分型 + 格式校验 + Ticks 比较 + StartTime 访问异常防护把误接管（假阳性）压到零：任何"原进程可能仍存活"或"证据形态不可信"或"进程实例信息不可读"的情况都判 LOCKED。
- **释放**：sync 末步或异常分支，确认 runId 匹配后删锁。

> **锁空转处置（v0.2.16 C1 修订）**：`LOCKED|` 统一涵盖独占/崩溃残留/锁文件消失。fail-closed → blocked，**不人工抢锁**。停摆窗口分两类：① 若持锁进程**确实终止**（PID 经伪代码证明死亡或 PID 被新进程复用），最坏 30 分钟自动恢复（下一轮 beat 超 30min 阈值时陈锁接管判 TAKEOVER 重争）；② 若持锁进程**存活但挂起**（网络 IO 死等/死循环/beat 停更但进程未死），PID+StartTime 校验持续判"仍存活→LOCKED"，**无自动上限**，需走 §6 恢复手册人工介入清锁（这与恢复手册作为硬约束章节的存在本身互为佐证——若真 30min 自愈则无需人工清锁手册）。
>
...
**Compare-SemVer 正规化与比较规则（P1 v0.2.11 P0-3，回应 GPT——消除实现期"同输入不同分支"验收争议）**：
...
| `upgrade-<cli>.ps1` | 读 local+remote → 判 behind（§7.3 if-elif）；ACTIONABLE=`mise upgrade <tool>@latest`（**v0.2.16 H2：实现期 MUST 实测此语法接受性——mise 官方 `upgrade` 子命令对 `@version` 参数支持未实测验证，若实测不支持则改用 `mise install <tool>@<remote.latest> --force`（与 REPAIR 同语法），实现期实测后补时间戳**），REPAIR=`mise install <tool>@<remote.latest> --force`（P0 v0.2.10 D4：`<ver>` 收敛为 `<remote.latest>`，与 §7.3 条件5/§5.3 对齐） | 读 remote → 下载（§5.2 完整性策略：digest 主路径 + checksum 解析 fallback）→ **ZipSlip 安全解压** → staging exe 计算 sha256 |
| `verify-<cli>.ps1` | 重扫 mise installs → 全字段 + runId(MUST) + sha256 SHOULD | 跑 staging exe `--version` → 全字段 + runId(MUST) + sha256 |
| `sync-v0.2.16.ps1`（共享） | 遍历三 CLI verified.json → 三重约束判定 → copy + 字节/sha256 复核 + `.previous` 备份 + 证据链 + summary 汇总 + opencode npm fallback 计数更新（§10.5） | 同左 |
...
读 `local.json` + `remote.json`（条件7/8 另需读 `verified.json` + `current-run.json` 判凭证新鲜度，P0 v0.2.10 D3 补全输入清单）→ **按下列顺序 if-elif 评估，命中即止（自上而下优先级）**：
...
| 3 | target 为 prerelease（**v0.2.16 B1：判据按通道分流**） | `TARGET_PRERELEASE\|<cli> <v>` | stable-only 拒绝，跳过 verify |
...
**条件7/8 互补与可达性（P0 v0.2.10 D3，回应 CodeBuddy——闭合"无分支命中"空洞）**：
- **时序前提**：§9 状态机每轮 `upgrade → verify` 顺序执行一次。upgrade 执行时点，**本轮 verify 尚未运行**——故 upgrade 读到的 `verified.json` 必然来自上一轮（runId/runAt 均为上轮），按 §5.4 新鲜度判据（P1 v0.2.13 CB-P2-4：runId 唯一必要条件，仅字段缺失回退 runAt）**跨轮首次调用恒不新鲜**（旧轮 runId ≠ 本轮）。
...
### 7.4 版本号
共享库与 sync 文件名含 `-v0.2.16`；probe/upgrade/verify 每 CLI 一份，首行 `$ScriptVersion='0.2.16'` + `# SPEC: v0.2.16`。
...
**标记行语法（P2 v0.2.15 GPT-P0-2，MUST——agent 解析的可靠性基础）**：
...
| `RUNTIME_ERROR_FATAL\|<cause>` | 任何段（骨架级：锁争取/续取失败、state 文件 schema 校验失败、汇总异常） | fatal | 整轮终止。P0 v0.2.10 D8：upgrade/verify 写 state 文件时回读校验失败（§5.8）亦产 FATAL，非仅 archive/sync |
| `RUNTIME_ERROR\|<cli> <cause>` | probe/upgrade/verify（CLI 级） | recoverable | 该 cli 失败，继续下一个 |
| `ARCHIVE_OK\|<runId>` | archive-state | 成功 | 争锁成功+归档完成（runId 已写 current-run.json）→ 进 probe（P2 v0.2.14 CH-P1-2：补齐 archive-state 成功标记——此前唯一无 `*_OK\|` 的段，agent 判 OK 依据悬空） |
...
> P2 v0.2.15（回应 GPT-P1-3）：脚本输入/输出见 §7.2、分支判定路径见 §7.3/§9 状态机、失败不覆盖规则见 §5.4/§5.3——本节不重复这些信息、不另设"单页总览"（并列事实源信息零新增且易漂移）。

| 脚本 | 关键标记 | exitCode | 文件副作用（写/覆盖/不写） |
|---|---|---|---|
| archive-state | `ARCHIVE_OK\|<runId>`（P2 v0.2.14 CH-P1-2） | 11 | 写 `current-run.json`（新建 runId）、`run.lock`、`archive/<ts>/` |
...
| probe-remote | `REMOTE_FAIL\|<cli>`/`RATE_LIMITED\|<cli>` | 3 | 写 `<cli>-remote.json`（P0 v0.2.10 D10：必填字段全落盘——latest/sourceUrl/directUrl/assetName/assetDigest/expectedSha256/checksumAssetUrl=null, fallbackUsed=false, error=<cause>） |
...
| probe-local/probe-remote/upgrade/verify | `RUNTIME_ERROR_FATAL\|schema` | 3 | P0 v0.2.10 D8：state 文件原子写入回读校验失败（§5.8）→ 释放锁、不写回原文件、整轮终止（非 CLI 级 recoverable） |
...
| sync | `SYNC_TARGET_LOCKED\|<cli>` | 0 | 该 CLI entry **action=target-locked**（P1 v0.2.13 CB-P2-3：与 §5.5 枚举同名钉死，不再写 action=skip），sync.json 仍写 entry（reason 必填） |
...
段A: archive + probe
  archive-state ──[LOCKED|/STATE_MISSING|/PARSE_ERROR|/RUNTIME_ERROR_FATAL]──→ fatal 终止
        │ OK（ARCHIVE_OK|<runId>，P2 v0.2.14 CH-P1-2）
        ▼
...
```json
{ "specVersion":"0.2.16", "name":"opencode", "consecutiveSyncSuccess":<int>,
  "lastUpdatedAt":"<ISO>", "uninstalled":false }
```
- **更新时机**：sync 末步（sync-v0.2.16.ps1 汇总后）更新。
...
- **失败归零**：本轮 opencode sync 失败（copy 失败/复核不过/目标锁定/三重约束不满足且非 already-current 的 `SYNC_SKIP`）→ `consecutiveSyncSuccess = 0`。**注**：`ALL_REMOTE_FAIL` 不属"未跑 sync"——§8/§9 明确其直进 sync，opencode 条目走 `SYNC_SKIP`（无新鲜凭证，非 already-current）即归零，已被本条覆盖。**已卸载后失败（P1 v0.2.13 CB-P2-1 修订）**：`uninstalled==true` 后失败**不归零**（完全冻结的一部分）——失败语义由 `sync.json` entries `ok=false` 与 `fetch_run.log` 记录；`uninstalled` 一旦 true 不回退。
- **P0 v0.2.10 D5 唯一规则（闭合与 §8.1 冲突；v0.2.16 F1 表述对齐细粒度）**：计数更新按下列**细粒度规则**（与 §8.1 `RUN_STATUS|failed` 行一致，本条为其唯一权威定义）：① sync 完整执行到末步（opencode 条目已处理）→ 按 +1/归零规则更新；② sync 内部 fatal 中断（`LOCKED|`/`STATE_MISSING|`/`PARSE_ERROR|`/`RUNTIME_ERROR_FATAL|` 在 sync 末步前触发）→ **若 opencode 条目已在 fatal 前处理过则按该结果更新（+1 或归零），否则不动**（fatal 前未跑到 opencode → 保持旧值）。fatal 不归零——避免一次骨架级中断清掉累计进度（仅 opencode 条目本身失败才归零）。原 v0.2.10/v0.2.15 文字"fatal 中断→不更新计数（保持旧值）"为粗粒度表述，已废除（v0.2.16 F1：粗粒度表述让只读 §10.5 的实现者遗漏"fatal 前已处理则更新"分支）。
- **卸载触发（v0.2.16 E1/E2 修订）**：`consecutiveSyncSuccess >= 10` 且 `uninstalled==false` → 执行 `npm uninstall -g opencode`，**MUST 检查退出码**：仅 npm 退出码==0 才置 `uninstalled=true`（幂等：已达 10 且 uninstalled=true 后续不再重复卸载）；npm 退出码!=0 / npm 二进制缺失 / 权限拒绝 → **保持 `uninstalled=false`**、记 fetch_run.log `npm uninstall failed: <cause>`、等待下一达标轮次重试（"一旦 true 不回退"是单向上限，失败侧不冻结——CB-P2-1 冻结规则仅适用 `uninstalled==true` 后）。**调用 npm MUST NOT 依赖 PATH**（§2 末行"harness 不依赖 PATH/mise shim"同源约束）：显式使用 `Get-NpmPrefix()`（§7.1 精确值 `E:\Users\WIN_11\AppData\Roaming\npm`）定位 npm 全局位置，调用该 prefix 下 npm 二进制或显式传 `--prefix <Get-NpmPrefix()>`（Cherry 沙箱下 npm 装非默认位置，依赖 PATH 解析存在操作错误 npm 实例或找不到目标包的风险）。
...
| 维度 | Software_Update_Monitor v0.2.2 | Release-Monitor | 本项目 v0.2.16 |
|---|---|---|---|
...
7. 现有散脚本（`.old/`）→ 按 §7 模块清单拆分重命名（-v0.2.16 / $ScriptVersion）。`.old/` legacy 脚本受仓库 AGPL-3.0 LICENSE 约束。**v0.2.16 H7（LICENSE 复用风险标注）**：新脚本实现期 MUST 明确是否对 `.old/` 代码有**实质复用**（直接移植函数逻辑、复制代码段等）——若有，新脚本沿用 AGPL-3.0 不变（同源衍生）；若纯重写（仅参考架构、不复用代码行），仍可保留 AGPL-3.0 或评估切换 LICENSE。本项目主体为个人维护流程、不面向外部发行，AGPL-3.0 传染风险对本流程无实质影响；但实现期 MUST 在 SOP 中记录"新脚本↔.old/ 复用判定"以闭合审计追溯。
...
## 14. 待确认 / 已知风险（v0.2.16 闭合状态，回应 anthropic v0.2.11 P0-1/P0-2 + GPT v0.2.10 P0-1/P0-2/P0-3 + CodeBuddy D1–D12 + CodeBuddy v0.2.12 审计 CB-P1~CB-P3 + Cherry v0.2.13 审计 CH-P1~CH-P2 + GPT v0.2.14 审计 GPT-P0~GPT-P1 + anthropic v0.2.15 审计 A1/A2/B1/C1/E1/E2/F1/G1/H1~H7）
...
- **单元**：`Compare-SemVer`（边界 + §7.1 测试表 12 条 + §7.3 if-elif 顺序全覆盖 + **v0.2.16 B1：github-binary 通道用 API `prerelease` 字段、mise 通道用字符串启发式的分流判定**）、`Invoke-Proc`（超时/Kill）、时间字段序列化/解析断言（UTC 'o' 恒 7 位小数 + `Z`；比较走 Ticks 层；负例断言 MUST NOT DateTimeOffset——P2 v0.2.15 GPT-P1-1）、健康检查分型、**`healthy`/`healthDetail` 相关性不变量断言（v0.2.16 G1：`healthy==true ⟺ healthDetail==ok`；破坏该相关性的状态 MUST 触发 `RUNTIME_ERROR_FATAL|schema`）**、SHA256 计算、ZipSlip 解压（构造恶意 entry）、checksum 文件解析（两格式/多资产匹配/空白注释行/同名 hash 冲突→checksum-mismatch）、**API digest 解析（v0.2.16 H5：digest 存在但前缀非 `sha256:` → 视为不可用回退 checksum asset）**、凭证新鲜度判据（runId 匹配即新鲜 / runId 存在但不匹配→runAt 不放行（时钟回拨模拟）/ runId 字段缺失→runAt 兜底）、pin 规则（修复性晋升 / pinned-mismatch / pin 不阻断 upgrade）、**Authorization 头脱敏断言（v0.2.16 H4：错误日志中 Authorization MUST 显示 `<redacted>`）**、**`sync.json.entries[].reason` 受控枚举断言（v0.2.16 H6：skip 类 ∈ {no-fresh-credential, pinned-mismatch, already-current}）**、sync 异常隔离（单 CLI copy 失败不冒泡）、**§10.5 npm 卸载失败不置 true 断言（v0.2.16 E1：npm 退出码!=0 → 保持 uninstalled=false）**、**§10.5 npm 调用不依赖 PATH 断言（v0.2.16 E2：MUST 用 `Get-NpmPrefix()`）**、**§10.5 计数细粒度规则断言（v0.2.16 F1：sync 内部 fatal 前 opencode 已处理→按结果更新；fatal 前未跑到 opencode→不动）**、**陈锁接管五分支全覆盖 + 异常分型/格式校验断言（P0 v0.2.13 CB-P1-1/CB-P1-2/CB-P3-2 + v0.2.16 A1：按 §6 伪代码断言——①查询异常分型：锁内 pid 为 null/非法（实测抛 `ParameterBinding*`）或权限拒绝等 → 未证明死亡→`LOCKED|`；②`Get-Process` 正常未命中（`ProcessCommandException` 分型）→TAKEOVER 允许重争；③命中且 tick 匹配（'o' 格式锁值，Ticks 层比较）→同一实例仍存活→`LOCKED|` 不抢；④命中但 tick 不匹配（锁值已过格式校验）→PID 被复用→TAKEOVER；⑤锁时间字段缺失/格式不合契约（本地偏移/精度截断/非字符串/解析失败）→保守 `LOCKED|`；**⑥ v0.2.16 A1 新增：`Get-Process` 成功但 `.StartTime` 访问抛 `Win32Exception`（权限拒绝/对象已释放）→ 异常归入 `$lookupUnknown` → `LOCKED|`**；⑦负例断言：同一时刻的 string 形态 `-eq` 比较（实测恒 False）MUST NOT 出现在实现中）**。
- **集成**：每 CLI probe→upgrade→verify 串；覆盖 §7.3 九分支 + checksum-mismatch/dual-hash-mismatch/zip-slip/RATE_LIMITED。
- **端到端**：archive→三 CLI cycle→sync；archive 段断言成功标记 `ARCHIVE_OK|<runId>`（P2 v0.2.14 CH-P1-2）；全段 stdout **标记行语法合规断言**（独占一行/行首即标记名/标记名 ∈ §8 枚举/除 `RUN_STATUS` 外单 `|`/日志行不以标记形态开头——P2 v0.2.15 GPT-P0-2）；覆盖成功/uptodate-refresh/网络全断/exe 锁定/verify 失败/pin 锁定（含修复性晋升）/被锁阻塞（含恢复手册）/限流九场景；穿插 re-probe（含 `UPTODATE_SKIP` 可达断言，CH-P2-1）。
...
| 凭证新鲜度 | `runId == current-run.runId`（唯一必要条件）；仅 runId 字段缺失时回退 `runAt ≥ startAt`（P1 v0.2.13 CB-P2-4 收紧：runId 不匹配则恒不新鲜，防时钟回拨放过跨轮旧凭证） |
...
| expectedSha256 | remote.json 字段，完整性校验期望值：digest 主路径（从 API digest 剥 `sha256:` 前缀取 64hex）或 checksum 路径（P2 v0.2.13 CB-P3-5：checksum 条目 hash 亦落此字段，来源由 assetDigest 是否为 null 区分） |
...
| run.lock | §6 运行时锁文件（JSON）：runId/start/beat/pid/processStartTimeUtc；schema 见 §5.9 run.lock 行（P2 v0.2.14 CH-P1-1） |
...
| 标记行语法 | §8 契约：标记独占一行、行首即标记名（禁前导时间戳/级别）、首个 `|` 切分且标记名 ∈ §8 表枚举、除 `RUN_STATUS` 外 payload MUST NOT 含 `|`、非标记日志行禁以标记形态开头（P2 v0.2.15 GPT-P0-2） |
| Cherry mise 环境 | `Get-CherryMiseEnv`（MISE_* 七键常量）+ `Get-MiseInstallsDir`/`Get-MiseExePath`/`Get-NpmPrefix` 精确常量（§7.1）；mise 子进程 MUST 带 MISE_* env，文件扫描不依赖 env（P2 v0.2.15 GPT-P0-1） |
