# CLI 自动升级 —— 项目规格 SPEC v0.2.8

> **版本**：SPEC-v0.2.8（2026-09-16）。
> **变更摘要（v0.2.7 → v0.2.8）**：架构不动。据 anthropic frozen-direction 评审（v0.2.7）复核后修订，消歧冻结前最后缺口——
> 🔴 **阻塞项采纳（§7.3 分支优先级歧义）**：补显式规则——条件2（版本不可解析）仅在 `local.version` 与 `remote.latest` 均非 null 时判定；`local.version==null`（staging 空/broken）不计入"不可解析"，直接下沉至条件4/5。原缺口：opencode staging 空→`version=null`→Compare-SemVer(null) 返回 incomparable→命中条件2 `VERSION_FORMAT_ERROR`，**永远走不到条件5 `REPAIR`，首次 seed 卡死**。此为影响核心分支逻辑正确性的真实漏洞（非过度设计），此前 7 轮评审未捕获，anthropic 首次揪出。
> 🟡 **matchesTarget 比较基准采纳（§5.4）**：补各分支 `matchesTarget` 基准——ACTIONABLE/REPAIR 对比 `upgrade.json.target`；UPTODATE_REFRESH（无 upgrade.json，P0-3）对比 `local.version`（==`remote.latest`，UPTODATE 前提）。
> 🟡 **specVersion 不匹配行为采纳（§5.8）**：补硬约束——state 文件 specVersion 与当前实现不一致→视为陈旧，MUST NOT 作本轮决策依据消费（probe 类本轮覆盖；verified.json 不作晋升凭证——runId 新鲜度天然拦跨轮旧凭证，specVersion 检查为第二道保险）。不 fatal 终止（本轮重新探测会覆盖，终止反破坏可用性）。
> 🟢 **锁崩溃已知风险采纳（§14）**：补"archive-state 争锁与 current-run.json 写入间崩溃→下一轮 LOCKED"到 §14 已知风险声明（fail-closed 覆盖，无安全风险，仅人工清锁成本）。
> **观察项回应**：anthropic 建议定义量化冻结退出标准——认同价值，属项目治理 meta（非 spec 契约条款），不写进 spec 主体；建议用户在评审驱动层定义（如"连续两轮评审 0 新增 P0/P1 才冻结"）。
...
**关于 LICENSE（修正事实矛盾）**：仓库根含 `LICENSE`（AGPL-3.0），系 initial commit 随归档 legacy 脚本（`.old/`）带入，**适用于 `.old/` 下历史代码**。本项目主体（SPEC + SOP + `*-v0.2.8.ps1` 新脚本）不面向外部发行、无贡献指南、不发布包；AGPL-3.0 不改变本流程的"个人维护"性质。
...
3. **脚本**（`*-v0.2.8.ps1`）——模块化实现。**当前状态：待产出**（评审通过后产出，穿插 re-probe）。
...
├── SPEC-v0.2.8.md                    # 本文件（契约）
├── cli-autoupdate-sop.md             # 执行 SOP（给定时 agent，待产出）
├── cli-common-v0.2.8.ps1             # 共享库
...
├── sync-v0.2.8.ps1                   # 共享单脚本；每 CLI copy 独立 try/catch
...
所有 JSON 顶层带 `specVersion`（`"0.2.8"`）、`name`。时间内部 UTC，展示 UTC+08:00。**完整字段级 schema 见 §5.9（与示例一一对应）**。
...
```json
{ "specVersion":"0.2.8", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.8", "name":"opencode", "channel":"github-binary", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.8", "name":"codex", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.8", "name":"codex", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.8", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.8", "name":"<cli-autoupdate>", "runId":"<uuid>", "startAt":"<UTCISO>", "archivedTo":"archive\\<ts>" }
```
注：`name` 此处为任务名（非 CLI 名），`startAt` 替代 `runAt`（表本轮起点）。

### 5.7 `<cli>.pin`（可选，手动锁定目标版本）
```json
{ "specVersion":"0.2.8", "name":"codex", "pinVersion":"0.152.0", "reason":"user manual rollback", "setAt":"..." }
```
...
- 读入 state 文件时若 `specVersion != 当前实现版本`（如读到 `"0.2.8"` 而本轮跑 `"0.2.8"`），**视为陈旧文件**：MUST NOT 作本轮决策依据消费（即本轮 probe/upgrade/verify/sync 的判定**不以旧 specVersion 文件内容为准**）。
- **行为分级**：
...
| `sync-v0.2.8.ps1`（共享） | 遍历三 CLI verified.json → 三重约束判定 → copy + 字节/sha256 复核 + `.previous` 备份 + 证据链 + summary 汇总 | 同左 |
...
### 7.4 版本号
共享库与 sync 文件名含 `-v0.2.8`；probe/upgrade/verify 每 CLI 一份，首行 `$ScriptVersion='0.2.8'` + `# SPEC: v0.2.8`。
...
| 维度 | Software_Update_Monitor v0.2.2 | Release-Monitor | 本项目 v0.2.8 |
|---|---|---|---|
...
7. 现有散脚本（`.old/`）→ 按 §7 模块清单拆分重命名（-v0.2.8 / $ScriptVersion）。`.old/` legacy 脚本受仓库 AGPL-3.0 LICENSE 约束，新脚本沿用同 LICENSE（仓库既有）。
...
## 14. 待确认 / 已知风险（v0.2.8 闭合状态，回应 GPT P0-2 + anthropic 🟢）
