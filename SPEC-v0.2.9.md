# CLI 自动升级 —— 项目规格 SPEC v0.2.9

> **版本**：SPEC-v0.2.9（2026-09-16）。
> **变更摘要（v0.2.8 → v0.2.9）**：架构不动。据 anthropic 冻结评审（v0.2.8）复核后修订，闭合"文本已写但语义未闭合"的契约缺口——
> 🔴 **P0-1 REPAIR target 语义采纳（§7.3/§5.3）**：补显式声明——REPAIR 的 `target = remote.latest`（与 ACTIONABLE 对齐，"既然要重装，顺便修到当前 stable 最新"）。原缺口：条件5 动作 `mise install <tool>@<ver> --force` / opencode 重下载解压，`<ver>` 取值未定义——opencode staging 空/损坏时**没有本地版本可重装**，必用 remote.latest，但 mise 通道语义未对齐，两通道行为可能不一致。连锁：§5.4 matchesTarget 基准"对比 upgrade.json.target"虽写但 target 该填什么未定义，checklist 验收机械化前提在 REPAIR 分支落空。
> 🔴 **P0-2 verified.json channel schema 矛盾采纳（§5.4/§5.9）**：§5.9 表 verified.json channel 必填，但 §5.4 示例无 channel 字段——schema/示例不一致。补示例 `"channel":"mise"`（verified.json 自带通道，sync 无需跨文件读 local/remote.json 联合判断 sha256 MUST/SHOULD）。
> 🟡 **P1-1 prerelease-vs-REPAIR 优先级边界采纳为§14 风险（§14-11）**：broken+remote 为 prerelease 时 TARGET_PRERELEASE 先命中→REPAIR 永远排不上号。此为 stable-only 策略必然后果（不应修到 prerelease），重排会与 stable-only 冲突；三 CLI 持续有 stable，概率极低。列§14 声明（对齐 §14-2/3/8 风格），不重排。
> 🟡 **P1-2 N=10 计数机制采纳（§10.5/§5）**：§10.5 称"已闭合"但无计数持久化字段，名实不符。补方案(a)轻量计数文件 `state/opencode-npm-fallback.json`（consecutiveSyncSuccess 计数，sync 末步更新，达 10 卸载置 uninstalled=true）。
> 🟢 **P1-3 ALL_REMOTE_FAIL 聚合澄清采纳（§8）**：补一句——RATE_LIMITED 与 REMOTE_FAIL 均计入"remote fail"，三 CLI 全失败即 ALL_REMOTE_FAIL。
...
**关于 LICENSE（修正事实矛盾）**：仓库根含 `LICENSE`（AGPL-3.0），系 initial commit 随归档 legacy 脚本（`.old/`）带入，**适用于 `.old/` 下历史代码**。本项目主体（SPEC + SOP + `*-v0.2.9.ps1` 新脚本）不面向外部发行、无贡献指南、不发布包；AGPL-3.0 不改变本流程的"个人维护"性质。
...
3. **脚本**（`*-v0.2.9.ps1`）——模块化实现。**当前状态：待产出**（评审通过后产出，穿插 re-probe）。
...
├── SPEC-v0.2.9.md                    # 本文件（契约）
├── cli-autoupdate-sop.md             # 执行 SOP（给定时 agent，待产出）
├── cli-common-v0.2.9.ps1             # 共享库
...
├── sync-v0.2.9.ps1                   # 共享单脚本；每 CLI copy 独立 try/catch
...
所有 JSON 顶层带 `specVersion`（`"0.2.9"`）、`name`。时间内部 UTC，展示 UTC+08:00。**完整字段级 schema 见 §5.9（与示例一一对应）**。
...
```json
{ "specVersion":"0.2.9", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.9", "name":"opencode", "channel":"github-binary", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.9", "name":"codex", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.9", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.9", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.9", "name":"<cli-autoupdate>", "runId":"<uuid>", "startAt":"<UTCISO>", "archivedTo":"archive\\<ts>" }
```
注：`name` 此处为任务名（非 CLI 名），`startAt` 替代 `runAt`（表本轮起点）。

### 5.7 `<cli>.pin`（可选，手动锁定目标版本）
```json
{ "specVersion":"0.2.9", "name":"codex", "pinVersion":"0.152.0", "reason":"user manual rollback", "setAt":"..." }
```
...
- 读入 state 文件时若 `specVersion != 当前实现版本`（如读到 `"0.2.9"` 而本轮跑 `"0.2.9"`），**视为陈旧文件**：MUST NOT 作本轮决策依据消费（即本轮 probe/upgrade/verify/sync 的判定**不以旧 specVersion 文件内容为准**）。
- **行为分级**：
...
| `sync-v0.2.9.ps1`（共享） | 遍历三 CLI verified.json → 三重约束判定 → copy + 字节/sha256 复核 + `.previous` 备份 + 证据链 + summary 汇总 + opencode npm fallback 计数更新（§10.5） | 同左 |
...
### 7.4 版本号
共享库与 sync 文件名含 `-v0.2.9`；probe/upgrade/verify 每 CLI 一份，首行 `$ScriptVersion='0.2.9'` + `# SPEC: v0.2.9`。
...
| `ALL_REMOTE_FAIL\|` | probe-remote 末 | 整轮级 | 三 CLI 全失败 → 跳 upgrade/verify，直进 sync。**聚合口径（🟢 v0.2.9，回应 anthropic P1-3）**：`RATE_LIMITED` 与 `REMOTE_FAIL` **均计入"remote fail"**——三 CLI 中无论失败标记是 RATE_LIMITED 还是 REMOTE_FAIL，只要三者均失败即触发 ALL_REMOTE_FAIL |
...
```json
{ "specVersion":"0.2.9", "name":"opencode", "consecutiveSyncSuccess":<int>,
  "lastUpdatedAt":"<ISO>", "uninstalled":false }
```
- **更新时机**：sync 末步（sync-v0.2.9.ps1 汇总后）更新。
- **成功计数**：本轮 opencode 条目 sync 成功（`SYNC_COPY` 或 `already-current` 幂等 skip）→ `consecutiveSyncSuccess += 1`。
- **失败归零**：本轮 opencode sync 失败（copy 失败/复核不过/目标锁定/三重约束不满足非 already-current 的 SYNC_SKIP/未跑 sync 如 ALL_REMOTE_FAIL）→ `consecutiveSyncSuccess = 0`。
...
| 维度 | Software_Update_Monitor v0.2.2 | Release-Monitor | 本项目 v0.2.9 |
|---|---|---|---|
...
7. 现有散脚本（`.old/`）→ 按 §7 模块清单拆分重命名（-v0.2.9 / $ScriptVersion）。`.old/` legacy 脚本受仓库 AGPL-3.0 LICENSE 约束，新脚本沿用同 LICENSE（仓库既有）。
...
## 14. 待确认 / 已知风险（v0.2.9 闭合状态，回应 GPT P0-2 + anthropic 🟢 + anthropic P1-1）
