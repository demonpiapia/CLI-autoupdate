# CLI 自动升级 —— 项目规格 SPEC v0.2.13

> **版本**：SPEC-v0.2.13（2026-09-16）。
> **变更摘要（v0.2.12 → v0.2.13）**：架构不动。据 CodeBuddy 独立审计（对 v0.2.12）复核，闭合 §6 伪代码两处 fail-open 缺口（误接管方向），全为契约级修订：
> - 🔴 **CB-P1-1**（采纳，实测复现）：§6 伪代码 catch 吞并**全部**异常——锁内 pid 为 null/非法（锁半写，§14-10 自认崩溃场景）实测抛 `ParameterBinding*` 而非"进程不存在"，被同路径误判"已死→TAKEOVER"。改**异常分型**：仅 `ProcessCommandException`（PID 无活跃进程）判死 → `TAKEOVER`；`ParameterBinding*`、权限拒绝等 → 未证明死亡 → `LOCKED`。
> - 🔴 **CB-P1-2**（采纳，实测复现）：§6 `-eq` 时间比较未规定序列化/归一化——实测 string 形态比较同一时刻恒 `False`（Kind 语义），落入 else → TAKEOVER 误接管。钉死：写侧 MUST UTC `'o'` round-trip（tick 精度、`Z` 后缀）；读侧 MUST 解析（RoundtripKind）+ 归一 `.ToUniversalTime()` + **Ticks 层比较**；锁值格式不合契约（本地偏移/精度截断/解析失败）→ 保守 `LOCKED`。
> - 🟡 **CB-P2-1**（采纳，选方案 b）：§10.5/§5.9 计数规则统一——`uninstalled==true` 后**完全冻结**（成功不增、失败不归零，定格 10），闭合"定格 10"与"失败仍归零"矛盾；失败语义由 sync.json entries.ok=false 与 fetch_run.log 记录。
> - 🟡 **CB-P2-2**（采纳）：§5.5 `runStatus` 语义收窄为"sync 本次执行的终态"；判"本轮完整成功" MUST 联判 `sync.json.runId == current-run.runId && runStatus=="success"`；fatal 时由 sync 异常分支**原子写入** `runStatus:"failed"` 版（entries=fatal 前已处理条目）；废除"单文件自洽/无字段=腰斩"过强声明。
> - 🟡 **CB-P2-3**（采纳）：`SYNC_TARGET_LOCKED` 落盘钉死 `action="target-locked"`（与标记同名，便于汇总统计）+ `reason` 必填；§8.1"entry=skip"表述与 §5.9"skip 必填"条件同步修订。
> - 🟡 **CB-P2-4**（采纳）：凭证新鲜度判据收紧——`runId` 匹配为**唯一必要条件**；仅 runId **字段缺失**（非不匹配）时回退 `runAt ≥ startAt`。闭合"或"判据在时钟回拨下放过跨轮旧凭证 + §1-11/§5.4/§14-8/§17 四处口径不一。
> - 🟢 **CB-P3-1**（采纳）：§9 pin 连续 mismatch 数据源明确（回溯 `state/archive/` 最近 N 轮 sync.json entries，不新增计数字段）。
> - 🟢 **CB-P3-2**（采纳）：分支计数统一为"四分支"（三方向 + 字段缺失 fallback），随 CB-P1-1/P1-2 扩充为含异常分型/格式校验的完整断言集。
> - 🟢 **CB-P3-3**（采纳，实测核实）：§0 仓库内容声明按实修正（工作区含验收 checklist 候选稿与历轮评审/工具目录；`.old/` 亦被 `.gitignore` 排除）。
> - 🟢 **CB-P3-4**（采纳，轻量）：§3/§4 注明实机目录为 `Codex`（大写 C），spec 文本统一小写 `codex` 为书写约定，Windows 大小写不敏感等价。
> - 🟢 **CB-P3-5**（采纳，强化）：§5.2 策略 3 checksum 路径期望 hash 落 `expectedSha256`（来源可由 `assetDigest==null && expectedSha256!=null` 机械区分 + fetch_run.log 记录），闭合审计证据链缺口。
...
**关于 LICENSE（修正事实矛盾）**：仓库根含 `LICENSE`（AGPL-3.0），系 initial commit 随归档 legacy 脚本（`.old/`）带入，**适用于 `.old/` 下历史代码**。本项目主体（SPEC + SOP + `*-v0.2.13.ps1` 新脚本）不面向外部发行、无贡献指南、不发布包；AGPL-3.0 不改变本流程的"个人维护"性质。
...
3. **脚本**（`*-v0.2.13.ps1`）——模块化实现。**当前状态：待产出**（评审通过后产出，穿插 re-probe）。
...
├── SPEC-v0.2.13.md                   # 本文件（契约）
├── cli-autoupdate-sop.md             # 执行 SOP（给定时 agent，待产出）
├── cli-common-v0.2.13.ps1            # 共享库
...
├── sync-v0.2.13.ps1                  # 共享单脚本；每 CLI copy 独立 try/catch
...
所有 JSON 顶层带 `specVersion`（`"0.2.13"`）、`name`。时间内部 UTC，展示 UTC+08:00。**完整字段级 schema 见 §5.9（与示例一一对应）**。
...
```json
{ "specVersion":"0.2.13", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.13", "name":"opencode", "channel":"github-binary", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.13", "name":"codex", "runAt":"...", "runId":"<SHOULD>",
...
```json
{ "specVersion":"0.2.13", "name":"codex", "channel":"mise", "runAt":"...", "runId":"<本轮uuid, MUST>",
...
```json
{ "specVersion":"0.2.13", "name":"sync", "runAt":"...", "runId":"<本轮uuid, MUST>",
  "runStatus":"success",
...
```json
{ "specVersion":"0.2.13", "name":"<cli-autoupdate>", "runId":"<uuid>", "startAt":"<UTCISO>", "archivedTo":"archive\\<ts>" }
```
注：`name` 此处为任务名（非 CLI 名），`startAt` 替代 `runAt`（表本轮起点）。

### 5.7 `<cli>.pin`（可选，手动锁定目标版本）
```json
{ "specVersion":"0.2.13", "name":"codex", "pinVersion":"0.152.0", "reason":"user manual rollback", "setAt":"..." }
```
...
- 读入 state 文件时若 `specVersion != 当前实现版本`（如读到 `"0.2.12"` 而本轮跑 `"0.2.13"`），**视为陈旧文件**：MUST NOT 作本轮决策依据消费（即本轮 probe/upgrade/verify/sync 的判定**不以旧 specVersion 文件内容为准**）。
- **行为分级**：
...
| 文件 | 字段 | 类型 | 必填 | 枚举/说明 |
|---|---|---|---|---|
| 全部 | specVersion | string | 是 | `"0.2.13"` |
...
- **争锁**（archive-state 首步）：`[IO.File]::Open(run.lock, FileMode::CreateNew, FileAccess::ReadWrite, FileShare::None)` 原子争锁；写 `runId/start/beat/pid/processStartTimeUtc`，runId 写入 `current-run.json`。🔴 **v0.2.12 P0-2（升 MUST）**：`processStartTimeUtc` **MUST 写入**（取争锁进程自身的 `Process.StartTime.ToUniversalTime()`）——陈锁接管逻辑 MUST 依赖此字段判进程实例同一性（旧版 v0.2.11 定 SHOULD 但接管段写 MUST 比对，字段缺失则无对象可比，契约断裂）。PID 可能被 OS 重用（短命进程退出后新进程恰好分到同 PID），`processStartTimeUtc` 是进程实例的唯一性副证。🔴 **v0.2.13 CB-P1-2（序列化钉死）**：写入时 MUST 序列化为 **UTC round-trip（`'o'`）格式**（如 `2026-09-16T02:11:03.1234567Z`，保留完整 tick 精度、`Z` 后缀）——读侧 Ticks 比较的前提；格式不合契约的锁值按"字段无效"保守处理（见伪代码格式校验前置）。
...
### 7.1 共享库 `cli-common-v0.2.13.ps1`
...
| `sync-v0.2.13.ps1`（共享） | 遍历三 CLI verified.json → 三重约束判定 → copy + 字节/sha256 复核 + `.previous` 备份 + 证据链 + summary 汇总 + opencode npm fallback 计数更新（§10.5） | 同左 |
...
### 7.4 版本号
共享库与 sync 文件名含 `-v0.2.13`；probe/upgrade/verify 每 CLI 一份，首行 `$ScriptVersion='0.2.13'` + `# SPEC: v0.2.13`。
...
| sync | `RUN_STATUS\|failed\|<cause>` | 2 | 仅 fatal 触发；🟡 v0.2.13 CB-P2-2：fatal 时由 sync 异常处理分支**原子写入** `runStatus:"failed"` 版 sync.json（runId=本轮，entries=fatal 前已处理条目，无则空数组——不存在"部分写"中间态，§5.8）；opencode-npm-fallback.json 计数按已跑的 opencode 条目结果更新（fatal 前已处理则更新，否则不动——§10.5 D5 唯一规则不变） |
...
```json
{ "specVersion":"0.2.13", "name":"opencode", "consecutiveSyncSuccess":<int>,
  "lastUpdatedAt":"<ISO>", "uninstalled":false }
```
- **更新时机**：sync 末步（sync-v0.2.13.ps1 汇总后）更新。
...
| 维度 | Software_Update_Monitor v0.2.2 | Release-Monitor | 本项目 v0.2.13 |
|---|---|---|---|
...
7. 现有散脚本（`.old/`）→ 按 §7 模块清单拆分重命名（-v0.2.13 / $ScriptVersion）。`.old/` legacy 脚本受仓库 AGPL-3.0 LICENSE 约束，新脚本沿用同 LICENSE（仓库既有）。
...
## 14. 待确认 / 已知风险（v0.2.13 闭合状态，回应 anthropic v0.2.11 P0-1/P0-2 + GPT v0.2.10 P0-1/P0-2/P0-3 + CodeBuddy D1–D12 + CodeBuddy v0.2.12 审计 CB-P1~CB-P3）
...
> **🟢 v0.2.12 P2-5（版本历史日期说明）**：v0.1–v0.2.13 均标注 2026-09-16——确系同日多轮密集迭代（设计日内跨模型审计收敛）。日期列精度为"日"，外部审计者如需迭代节奏，可按版本号顺序（每轮一行）追溯；后续进入实现期的 SOP/脚本版本将带时分。
