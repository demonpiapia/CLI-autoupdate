...
> **版本**：SPEC-v0.2.18-rc3（2026-09-17）
> 用户指令：
...
├── SPEC-v0.2.18-rc3.md               # 本文件（契约）；v0.2.18 主线文件 = SPEC-v0.2.18.md
...
每状态文件落盘走「写 `*.tmp` → 回读校验关键字段 → `Move-Item` 原子替换」。同卷原子；跨卷先 Copy 到目标卷临时文件再同卷 Move。**原子替换实现建议（SHOULD）**：优先 .NET `File.Replace`（同卷）或等价原子替换 API；`Move-Item -Force` 在本节为语义描述——其对已存在文件的覆盖异常行为随版本/提供程序而异，验收以结果断言为准（成功后旧文件不再可见、失败时原文件完整）。校验失败 → `RUNTIME_ERROR_FATAL|schema`、释放锁、终止整轮。
...
| API asset digest    | GitHub Releases API `.assets[].digest`（`sha256:<64hex>`），GitHub 计算并经 API 返回（证明下载 zip 与 API 所见 asset 一致，非签名/上游真实性保证，asset 重传/上游被攻破不防，§5.10）；opencode 完整性主路径 MUST（已实测 anomalyco/opencode 全 asset 均带 digest，§5.2）                                                     | <br />                                               | <br />                                 |
